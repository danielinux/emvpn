...
int demo_get_key(char *user, struct emvpn_key *key)
{
    ...
    const uint8_t secret_iv[VPN_IV_LEN] = "0123456789012345";
    memcpy(key->key, secret, VPN_KEY_LEN);
    memcpy(key->iv, secret_iv, VPN_IV_LEN);
    return 0;
}
...
{
#ifdef CRYPTO_NONE
    return crypto_none_init();
#endif
#ifdef CRYPTO_CTAOCRYPT
    return crypto_ctaocrypt_init();
#endif
#ifdef CRYPTO_OPENSSL
    return crypto_openssl_init();
#endif
    return -1;
}
